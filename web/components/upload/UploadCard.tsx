      ...
      <div className="space-y-4">
        ...
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-6 shadow-sm">
          ...
          <p className="mt-4 text-center text-xs text-slate-400">
            ...
            <a href="/pricing" className="text-brand-600 underline">See plans</a>
          </p>
        </div>
      </div>
    );
        ...
        {state.status === "processing" ? (
          <>
            ...
            <p className="mt-3 text-sm font-semibold text-slate-700">
              Processing <span className="text-slate-900">{state.fileName}</span>…
            </p>
            <p className="mt-1 text-xs text-slate-400">Extracting transactions · usually under 15s</p>
          </>
        ) : state.status === "error" ? (
          <>
            <AlertTriangle className="h-10 w-10 text-red-400" />
            <p className="mt-3 text-sm font-semibold text-red-700">{state.message}</p>
            <button onClick={(e) => { e.stopPropagation(); reset(); }}
              className="mt-3 flex items-center gap-1 text-xs text-slate-500 underline">
          ...
          <>
            <div className={cn(
              ...
              isDragActive ? "bg-brand-100" : "bg-brand-50 ring-2 ring-brand-100"
            )}>
              {isDragActive
            ...
            <p className="mt-1 text-sm text-slate-400">
              or <span className="font-medium text-brand-600 underline underline-offset-2">browse files</span>
            </p>
            <p className="mt-2 text-xs text-slate-400">PDF only · max {MAX_FILE_MB} MB · All Indian banks</p>

            {billing.tier === "FREE" && freeRemaining > 0 && (
              <div className="mt-4 rounded-full bg-emerald-50 px-4 py-1.5 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-200">
