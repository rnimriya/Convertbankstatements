  ...
  if (tier !== "FREE") {
    ...
    return (
      <div className={`flex items-center gap-4 px-4 py-3 rounded-xl border bg-brand-bg ${low ? "border-amber-200 dark:border-amber-700" : "border-brand-border"}`}>
        ...
        {low && (
          <a href="/pricing" className="shrink-0 text-xs font-semibold text-navy dark:text-violet-400 border border-navy/20 dark:border-brand-400/30 bg-zinc-900 dark:bg-zinc-950/5 dark:bg-brand-400/5 px-2.5 py-1 rounded-lg hover:bg-zinc-900 dark:bg-zinc-950/10 dark:hover:bg-brand-400/10 transition-colors">
            Upgrade
          </a>
        )}
      </div>
    ...
    return (
      <div className={`flex items-center gap-4 px-4 py-3 rounded-xl border ${isDepleted ? "border-amber-200 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/20" : "border-brand-border bg-brand-bg"}`}>
        ...
        {isDepleted ? (
          ...
          <div className="shrink-0 text-2xl font-bold tabular-nums text-navy dark:text-violet-400">
            {remaining}
          </div>
        )}
      </div>
