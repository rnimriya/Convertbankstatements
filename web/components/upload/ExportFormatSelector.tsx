      ...
      {FORMATS.map(({ id, label, sub, Icon }) => {
        ...
        return (
          ...
          >
            {/* Active check badge */}
            {active && (
              <div className="absolute top-2 right-2 w-4 h-4 rounded-full bg-brand-bg/20 flex items-center justify-center">
            ...
            <div>
              <p className={`text-sm font-bold leading-tight ${active ?"text-white" :"text-brand-text"}`}>
                {label}
              </p>
              <p className={`text-[10px] mt-0.5 leading-tight ${active ?"text-white/65" :"text-brand-muted"}`}>
                {sub}
              </p>
            </div>
          </button>
