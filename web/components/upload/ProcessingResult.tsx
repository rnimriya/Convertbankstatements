  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/20 p-5 shadow-sm">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="font-bold text-zinc-900 dark:text-gray-200">Processing complete!</h3>
              <Badge variant="success" size="sm" dot>Done</Badge>
            </div>
            <p className="mt-0.5 truncate text-sm text-zinc-500 dark:text-gray-400">{result.file_name}</p>
          </div>
        </div>
        ...
        {result.bank_name && (
          <p className="mt-3 text-center text-xs text-zinc-400 dark:text-gray-500">
            Detected: <span className="font-semibold text-zinc-600 dark:text-gray-300">{result.bank_name}</span>
          </p>
        )}
      ...
      {result.transactions.length > 0 && (
        <div className="rounded-2xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-surface overflow-hidden">
          <div className="flex items-center justify-between border-b border-zinc-100 dark:border-white/10 px-5 py-3 bg-zinc-50">
            <div className="flex items-center gap-2 text-sm font-semibold text-zinc-900 dark:text-gray-200">
              <TrendingUp className="h-4 w-4 text-zinc-500 dark:text-brand-400" />
              Transaction preview
            </div>
          ...
          <div className="divide-y divide-zinc-100 dark:divide-white/10 font-mono">
            {result.transactions.slice(0, 8).map((txn: Transaction, i) => (
              <div key={i} className="flex items-center gap-3 px-5 py-2.5 hover:bg-zinc-50 transition-colors">
                <div className="min-w-0 flex-1">
                  <p className="truncate text-xs font-medium text-zinc-900 dark:text-gray-200">{txn.description}</p>
                  <p className="text-xs text-zinc-500 dark:text-gray-500">
                    {txn.date}{txn.category && ` · ${txn.category}`}
                  </p>
                </div>
                <span className={`shrink-0 text-xs font-bold tabular-nums ${txn.amount >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-zinc-900 dark:text-gray-300"}`}>
