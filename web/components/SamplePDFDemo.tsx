...
export function SamplePDFDemo() {
  ...
  // done
  return (
    <div className="w-full max-w-[420px] bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-lg overflow-hidden">
      {/* Header */}
      ...
      {/* Stats */}
      <div className="grid grid-cols-3 divide-x divide-slate-100 dark:divide-white/10 border-b border-zinc-100 dark:border-zinc-800">
        <div className="px-4 py-3 text-center">
          <p className="text-lg font-bold text-zinc-900 dark:text-white">{result?.transaction_count}</p>
          <p className="text-[10px] text-zinc-400 dark:text-zinc-500 uppercase tracking-wide">Transactions</p>
        </div>
        <div className="px-4 py-3 text-center">
      ...
      <div className="px-4 pt-3 pb-1">
        <p className="text-[10px] font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-wide mb-2">Preview (first 3 rows)</p>
        <div className="overflow-x-auto rounded-lg border border-zinc-100 dark:border-zinc-800 text-xs">
          <table className="w-full">
            <thead>
              <tr className="bg-zinc-50 dark:bg-zinc-950">
                <th className="px-2.5 py-2 text-left text-zinc-500 dark:text-zinc-400 font-semibold">Date</th>
                <th className="px-2.5 py-2 text-left text-zinc-500 dark:text-zinc-400 font-semibold">Description</th>
                <th className="px-2.5 py-2 text-right text-zinc-500 dark:text-zinc-400 font-semibold">Amount</th>
              </tr>
            </thead>
            <tbody>
      ...
      <p className="px-4 pb-3 text-[10px] text-zinc-400 dark:text-zinc-500 text-center">
        Demo data — <a href="/signup" className="underline underline-offset-1 hover:text-zinc-900 dark:text-zinc-100 dark:hover:text-violet-400 transition-colors">sign up free</a> to convert your real statements
      </p>
    </div>
  );
}
