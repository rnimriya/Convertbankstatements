      ...
      <div className="rounded-2xl border border-brand-border bg-brand-surface p-5 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-brand-text">Team seats</p>
              <p className="text-xs text-brand-muted">{seatsFilled} of {maxSeats} seats used</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-2xl font-bold text-brand-text">{seatsLeft}</p>
            <p className="text-xs text-brand-muted">seats remaining</p>
          </div>
        </div>

        {/* Seat usage bar */}
        <div className="h-2 bg-zinc-100 dark:bg-brand-bg/10 rounded-full overflow-hidden">
          <div
...
function MemberRow({ email, name, role, status, joinedAt, onRemove, removing }: {
  ...
  return (
    <div className="flex items-center gap-3 px-5 py-3.5">
      <div className="h-8 w-8 rounded-lg bg-zinc-100 dark:bg-brand-bg/10 flex items-center justify-center text-xs font-bold text-zinc-600 dark:text-zinc-300 shrink-0">
        {initial}
      </div>
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 flex-wrap">
          <p className="text-sm font-medium text-brand-text truncate">{name ?? email}</p>
          {role ==="owner" && (
          ...
          {status ==="pending" && (
            <span className="text-[10px] font-bold text-brand-muted bg-zinc-100 dark:bg-brand-bg/10 px-1.5 py-0.5 rounded-full">
              Pending
            </span>
          )}
        </div>
        <p className="text-xs text-brand-muted">{name ? email :""} · Joined {date}</p>
      </div>
      {onRemove && role !=="owner" && (
        <button
          onClick={onRemove}
          disabled={removing}
          className="p-1.5 rounded-lg text-brand-muted hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors disabled:opacity-50"
          title="Remove member"
        >
