...
export function UsageHistory({ logs, isDemo }: { logs: Log[]; isDemo?: boolean }) {
  if (logs.length === 0) {
    return (
      <div className="flex flex-col items-center rounded-2xl border border-dashed border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 py-16 text-center">
        {isDemo ? (
          <>
  ...
  return (
    <div className="space-y-3">
      {logs.map((log) => {
        const badge = BILLING_BADGE[log.billingType] ?? { label: log.billingType, cls: "bg-slate-100 dark:bg-gray-700 text-slate-600 dark:text-gray-300" };
        const date = new Date(log.createdAt);
        return (
          <div
            key={log.id}
            className="flex items-center gap-4 rounded-2xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-5 py-4 shadow-sm hover:shadow-md dark:hover:shadow-black/20 transition-shadow"
          >
